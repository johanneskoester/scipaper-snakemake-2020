...
\begin{align*}
	\text{maximize } & T \cdot S \cdot \sum_{j \in J} x_j \cdot P_j + S \cdot \sum_{j \in J} x_j \cdot (T_j+1) + \sum_{f \in F} i_f \cdot s_f \\ \text{subject to } & x_j \in \{0,1\}\\ \quad & i_j \in [0,1].
\end{align*}
