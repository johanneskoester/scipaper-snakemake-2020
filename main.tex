...
Several of these features are shared with other tools (e.g., Conda and container integration), while others are (at the time of writing) exclusive to Snakemake (e.g., the scheduling mechanism, the graph partitioning, the caching mechanism). Some features of other tools are (not yet) included in Snakemake.

We explicitly refrain from performing a full comparison with other tools, as we believe such a summary will never be unbiased or lasting. Readers wishing to select a workflow system should instead consult current documentation, review recent articles, and personally try and evaluate each system for their individual needs.
