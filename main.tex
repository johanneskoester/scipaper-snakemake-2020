...
Within the latter partition, all jobs that have only incoming edges from the partition of finished jobs (or no incoming edge et all) can be scheduled next.
...
Third, temporary output files should be quickly (Snakemake allows output files to be marked as temporary, which leads to their automatic deletion once all consuming jobs have been finished).
...
Wwe assume that the red job at the root generates a temporary file, which may be deleted once all blue jobs are finished.
...
Let $F$ be the set of temporary files that will be generated during the workflow. \question{or: that are currently present / present when the current instance of the scheduling problem will be solved. ??}
%% Constants
...
For each temporary file $f \in F$, we define a variable $\delta_f \in [0,1]$ indicating the fraction of consuming jobs that will be scheduled among all pending jobs.
...
\begin{align}
\text{Maximize}\quad
    & 2U_\cores \cdot 2S \cdot \sum_{j \in J} x_j \cdot p_j 
  +   2S \cdot \sum_{j \in J} x_j \cdot (u_{\cores,j} + 1)  \nonumber \\
{}+ & S \cdot \sum_{f \in F} \gamma_f \cdot S_f 
  +   \sum_{f \in F} \delta_f \cdot S_f   \label{objective}
\end{align}
subject to
\begin{align}
...
\sum_{j \in J} x_j \cdot u_{r,j} &\leq U_r && \fueralle r \in R,
    \label{cn:resources}\\
\delta_f & \leq \frac{\sum_{j \in J}\, x_j \cdot z_{f,j}}{\sum_{j \in J}\, z_{f,j}} && \fueralle f \in F,
    \label{cn:lifetmp}\\
...
z_{f,j}\in\{0,1\} :& \quad \text{does job~$j$ need file~$f$?} \\
...
Constraint \eqref{cn:lifetmp} (together with the fact that $\delta_f$ occurs in the maximization) ensures that $\delta_{f}$ is ineed the lifetime fraction of temporary file $f \in F$.
Constraint \eqref{cn:deltmp} (together with the fact that $\gamma_f$ occurs in the maximization) ensures that $\gamma_f=0$ if and only if $\delta_f < 1$ and hence calculates whether temporary file $f \in F$ can be deleted.
