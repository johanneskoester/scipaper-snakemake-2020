...
We first define constants for each job $j \in J$: let $p_j \in \N$ be its priority, let $u_{r,j} \in \N$ be its usage of resource $r \in R$, and let $z_{f,j} \in \{0,1\}$ indicate whether it consumes temporary file $f \in F$.
...
\begin{align*}
	\text{minimize } & T \cdot S \cdot \sum_{j \in J} x_j \cdot p_j + S \cdot \sum_{j \in J} x_j \cdot (u_{t,j+1}) + \sum_{f \in F} \delta_f \cdot s_f \\ \text{subject to } & x_j \in \{0,1\}\\ \quad & \delta_j \in [0,1].
\end{align*}

In essence, jobs are selected such that jobs those with highest priority are preferred.
Upon ties, jobs are selected upon their number of threads.
Upon ties, jobs are preferred if they lead to closing temp files.
