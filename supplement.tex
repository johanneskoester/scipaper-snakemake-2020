...
In the example (\autoref{fig:design-patterns}e), we define that the \lstinline!qc! rule shall be a so-called \lstinline!checkpoint!.
...
\paragraph{Benchmarking (f)}
Sometimes, a data analysis entails the benchmarking of certain tools in terms of runtime, CPU, and memory consumption.
Snakemake directly supports such benchmarking by defining a \lstinline!benchmark! directive in a rule (line 7, \autoref{fig:design-patterns}f).
