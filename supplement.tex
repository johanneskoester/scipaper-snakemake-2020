...
\paragraph{Streaming.} Snakemake allows to stream output between jobs, instead of writing it to disk (see \autoref{m-sec:streaming}). Here, the output of rule \lstinline!step1! and \lstinline!step2! is streamed into rule \lstinline!step3!.

\paragraph{Non-file parameters.} Often, data analysis steps need additional non-file input in the form of parameters, that are for example obtained from the workflow configuration (see \autoref{m-sec:automation}).
